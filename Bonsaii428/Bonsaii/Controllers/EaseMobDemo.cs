...
namespace HXComm
{
    ...
    public class EaseMobDemo
    {
        ...
        //加好友
        public string AccountAdd(string username, string friend_username) { return ReqUrl(easeMobUrl + "users/" + username + "/contacts/users/" + friend_username, "POST", string.Empty, token); }
        ...
        public string AccountLookFriends(string userName) { return ReqUrl(easeMobUrl + "users/" + userName + "/contacts/users", "Get", string.Empty, token); }
        public string AccountDelFriend(string userName, string friendName) { return ReqUrl(easeMobUrl + "users/" + userName + "/contacts/users/" + friendName, "DELETE", string.Empty, token); }
            ...
            using (StreamReader sr = new StreamReader(txtFile))
            {
                ...
                while (line != null)
                {
                    ...
                    if (arr != null && arr.Length == 2)
                    {
                        ...
                        string returnMsg = easeMob.AccountCreate(u, p,"new");
