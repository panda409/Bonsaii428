...
namespace ConsoleTest
{
    class CheckingInManage
    {
        ...
        /// 值班是否需要打卡
        /// </summary>
        public bool OnDutyNeedSignIn { get; set; }
        /// <summary>
        /// 值班打卡的提前分钟数
        /// </summary>
        public int OnDutyAheadMinutes { get; set; }
        /// <summary>
        /// 值班打卡的推后分钟数
        /// </summary>
        public int OnDutyBackMinutes { get; set; }
        /// <summary>
        /// 旷职分钟数
        /// </summary>
        ...
        public CheckingInManage()
        {
            ...
            this.OnDutyNeedSignIn = db.CheckingInParamsBools.Find(3).Value;
            this.AbsentismMinutes = db.CheckingInParams.Find(9).Value;
            this.AbsentismHour = db.CheckingInParams.Find(10).Value;

            this.OnDutyAheadMinutes = db.CheckingInParams.Find(11).Value;
            this.OnDutyBackMinutes = db.CheckingInParams.Find(12).Value;
        }
        ...
        {
            int month = 90;
            DateTime dt = new DateTime(2015, 12, 3);
            for (int i = 0; i < month; i++)
            {
                DateTime CurrentDate = dt.AddDays(i);
                Console.WriteLine(CurrentDate);
                this.GenerateSignInCardStatusAndEveryDaySign(CurrentDate);
                this.CheckVacateApplies(CurrentDate);
                this.CheckEvectionApplies(CurrentDate);
    ...
    //        this.CheckOvertimeApplies(CurrentDate);
            ...
            this.CheckOnDutyApplies(CurrentDate);

            this.CheckDaysOffApplies(CurrentDate);
        ...
        public void CalculateDay()
        {
            ...
            this.CalculateChargeCard(currentDate);
            this.CalculateEvection(currentDate);
            this.CalculateOvertime(currentDate);
            //只有针对值班需要打卡的情况才要计算
            if (this.OnDutyNeedSignIn)
               this.CalculateOnDuty(currentDate);
            //请假不需要打卡，也就不用计算了
            List<string> staffNumbers = db.Staffs.Select(p=>p.StaffNumber).ToList();
            foreach (string tmpStaffNumber in staffNumbers)
                this.CalculateEveryDaySignInData(currentDate, tmpStaffNumber);
        }
        ...
        public void CheckEvectionApplies(DateTime CurrentDate)
        {
            ...
            foreach (EvectionApplies tmp in list)
            {
                ...
                //需要打卡
                if (this.EvectionNeedSignIn)
                {
                    //更改打卡的类型
                    foreach (SignInCardStatus tmpSignIn in tmpList)
                    {
                        tmpSignIn.SignInType = SignInType.EVECTION;
                        tmpSignIn.NeedStartTime = Tools.MinusMinutes(tmpSignIn.NeedWorkTime, this.EvectionAheadMinutes);
                        tmpSignIn.NeedEndTime = Tools.AddMinutes(tmpSignIn.NeedWorkTime, this.EvectionBackMinutes);
                    }
                }
                //不用打卡,就直接删除打卡点就行
                else
        ...
        public void CalculateEvection(DateTime CurrentDate)
        {
        ...
        public void CalculateOnDuty(DateTime CurrentDate)
        {
     ...
     //       List<EveryDaySignInDate> EveryDays = db.EveryDaySignInDates.Where(p => p.Date == CurrentDate).ToList();
            foreach (string StaffNumber in staffNumbers)
            {
             ...
             //       EveryDaySignInDate everydaysignindate = null;
                    try
                    {
                        ...
                        double totalHours = onDutyApply.Hours - this.CalculateAbsentHours(totallateminutes, totalearlyminutes) ?? 0;
                        //找到这个员工的值班时数，更新它
                        if (onDutyHours != null)
                        {
        ...
        /// </summary>
        /// 
        public void CalculateChargeCard(DateTime CurrentDate)
        {
            /**
             * 签卡的处理逻辑目前为：
             * 1、首先获取所有未处理的签卡请求
            ...
            {
                List<SignInCardStatus> SignIns = db.SignInCardStatus.Where(p => p.WorkDate == tmp.Date && p.StaffNumber.Equals(tmp.StaffNumber)).OrderBy(p => p.NeedWorkTime).ToList();
        ...
        /// 针对签卡过程，重新计算日考勤报表
        /// </summary>
        public void ReCalculateEveryDaySignIn(DateTime CurrentDate, string StaffNumber)
        {
            //先处理正常上班的情况
        ...
        /// </summary>
        /// <param name="CurrentDate"></param>
        public void ReCalculateOvertime(DateTime CurrentDate, string StaffNumber)
        {
        ...
        public void ReCalculateEvection(DateTime CurrentDate, string StaffNumber)
        {
